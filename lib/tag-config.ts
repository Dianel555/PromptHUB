...
export const ANALYSIS_RULES = {
  ...
  keywordExtraction: {
    ...
    stopWords: ['的', '了', '在', '是', '有', '和', '与', '或', '但', '而', '因为', '所以'],
  },
...
// 评估标签质量
export function evaluateTagQuality(confidence: number): 'excellent' | 'good' | 'acceptable' | 'poor' {
...
export function validateTag(tag: string): { valid: boolean; reason?: string } {
  ...
  if (tag.length > config.maxTagLength) {
    return { valid: false, reason: `标签长度不能超过${config.maxTagLength}个字符` }
  }
