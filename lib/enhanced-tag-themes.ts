...
// 根据维度获取颜色
export function getDimensionColor(dimension: string, theme: TagThemeConfig): string {
  ...
  return dimensionColors[dimension as keyof typeof dimensionColors] || theme.colors.primary
}
...
export function generateTagClasses(
  ...
  const variantClasses = {
    ...
    gradient: ['text-white', 'shadow-sm', 'hover:shadow-md', 'bg-gradient-to-r']
  }
...
// 响应式标签尺寸
export function getResponsiveTagSize(screenSize: 'mobile' | 'tablet' | 'desktop'): 'xs' | 'sm' | 'md' | 'lg' | 'xl' {
  const sizeMap = {
    mobile: 'xs' as const,
    tablet: 'sm' as const,
    desktop: 'md' as const
