...
export const tagSystemTranslations = {
  zh: {
    ...
    "tagEditor.smartSuggestionsDesc":
      "基于内容分析的标签建议，点击添加到标签列表",
    ...
    "tip.sameContentSameColor":
      "相同内容的标签会显示相同颜色，不同内容会自动分配不同颜色，确保视觉层次清晰",
  ...
  en: {
    ...
    "tagEditor.smartSuggestionsDesc":
      "Tag suggestions based on content analysis, click to add",
    "tagEditor.analyzing": "Analyzing content...",
    "tagEditor.noSuggestions":
      "No suggestions available, add more content for better suggestions",
    ...
    "tip.sameContentSameColor":
      "Same content tags show same color, different content gets different colors automatically for clear visual hierarchy",
...
export function useI18n(language: "zh" | "en" = "zh") {
  const t = (key: string): string => {
    return (
      (tagSystemTranslations[language] as Record<string, string>)[key] || key
    )
  }
...
export const languageStorage = {
  ...
  get(): "zh" | "en" {
    if (typeof window === "undefined") return "zh"
    return (
      (localStorage.getItem(this.key) as "zh" | "en") || detectBrowserLanguage()
    )
  },
