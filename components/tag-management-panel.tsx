...
import { TagCategory, TagDimension, tagDimensions } from "@/lib/tag-system"
    ...
    <Card key={category.id} className="transition-shadow hover:shadow-md">
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex items-center justify-between text-xs text-gray-500">
          <span>
            {language === "zh" ? "权重" : "Weight"}: {category.weight}
          </span>
          <span>
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                onCheckedChange={(checked) =>
                  onLanguageChange?.(checked ? "zh" : "en")
                }
              />
          ...
          <CardContent>
            <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              {renderStatsCard(
                ...
                language === "zh"
                  ? "已配置的标签维度"
                  : "Configured tag dimensions"
              )}
              {renderStatsCard(
                language === "zh" ? "最热标签" : "Top Tag",
                tagStats.mostUsedTags[0]?.name || "-",
                language === "zh"
                  ? `使用 ${tagStats.mostUsedTags[0]?.count || 0} 次`
                  : `Used ${tagStats.mostUsedTags[0]?.count || 0} times`
              )}
              {renderStatsCard(
        ...
        <CardContent>
          {filteredCategories.length > 0 ? (
            ...
            <div className="py-8 text-center">
              <Filter className="mx-auto mb-2 size-12 text-gray-300" />
              <p className="text-gray-500">
                {language === "zh"
                  ? "没有找到匹配的标签类别"
                  : "No matching tag categories found"}
              </p>
            </div>
          )}
        </CardContent>
